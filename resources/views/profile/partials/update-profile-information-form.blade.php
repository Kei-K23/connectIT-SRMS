<section>
    ...
    @if ($user->image)
    <div class="mt-5 w-[100px] h-[100px] bg-gray-200 rounded-full flex items-center justify-center overflow-hidden">
        <img class="object-contain w-full h-full" src="{{ asset('storage/' . $user->image) }}" alt="{{ $user->name }}">
    </div>
    @endif
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            ...
            <x-text-input id="address" name="address" type="text" class="block w-full mt-1"
                :value="old('address', $user->address)" autofocus autocomplete="address" placeholder="Address" />
        ...
        <div>
            ...
            <x-text-input id="image" name="image" type="file" class="block w-full mt-1"
                :value="old('image', $user->image)" autocomplete="image" placeholder="Profile image" />
